...
import {BAD, GOOD, YEAH, BEST, SUMMER} from './emoji';
...
class EmojiRain extends Component {
  ...
  componentDidMount() {
    ...
    const {drops = 250, active = true, score = 0} = this.props;
    ...
    if (this.score === 100) { this.__emoji = BEST; }
    else if (this.score >= 85) { this.__emoji = YEAH; }
    else if (this.score >= 40) { this.__emoji = GOOD; }
    else { this.__emoji = BAD; }

    this.__emoji.push(...SUMMER);

    // console.log(this.__emoji[0]);
